...
class TestOutputDisplayFunctions(unittest.TestCase):
    ...
    def test_decimal_to_binary(self):  # 0b is removed from bin result for all unit tests
        self.assertEqual((int(display.decimal_to_binary(6))),
                         (int(bin(6).replace("0b", ""))))  # Test for positive integer
        self.assertEqual((int(display.decimal_to_binary(-6))),
                         (int(bin(-6).replace("0b", ""))))  # Test for negative integer
        self.assertEqual(float(display.decimal_to_binary(8.25)), float(1000.01))  # Test for positive fraction
        self.assertEqual(float(display.decimal_to_binary(-6.75)), float(-110.11))  # Test for negative fraction

    def test_binary_to_decimal(self):
        self.assertEqual(display.binary_to_decimal(10011111011101),
                         10205)  # Test for converting positive binary number
        self.assertEqual(display.binary_to_decimal(1000.01), 8.25)  # Test for converting positive binary numbers
        # with fractions
        self.assertEqual(display.binary_to_decimal(-10), -2)  # Test for converting positive binary numbers with
        # fractions
        self.assertEqual(display.binary_to_decimal(-101111.101), -47.625)  # Test for converting positive binary
