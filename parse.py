...
class Parser(object):
    ...
    def evaluate_stack(self: object, symbol_stack: list, is_binary: bool) -> str:
        ...
        Args:
            ...
            is_binary (bool): If the input is binary or not
        ...
        # Process unary '-'
        if symbol == 'unary -':
            return -self.evaluate_stack(symbol_stack, is_binary)
        # Process other unary operators
        if symbol == "!":
            operand = self.evaluate_stack(symbol_stack, is_binary)
            return self.operation_map[symbol](operand)
        # Process binary operators
        operation = self.operation_map.get(symbol, False)
        if operation:
            operand_2 = self.evaluate_stack(symbol_stack, is_binary)
            operand_1 = self.evaluate_stack(symbol_stack, is_binary)
        ...
        if operation:
            ...
            args = reversed([self.evaluate_stack(symbol_stack, is_binary)
    ...
    def evaluate(self: object, expression: str, is_binary: bool) -> float:
        ...
        try:
            ...
            return self.evaluate_stack(self._symbol_stack[:], is_binary)
